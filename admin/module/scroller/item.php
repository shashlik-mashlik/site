...
if(is_numeric($_GET['deletep'])) {
	$sql = "SELECT `pid` FROM `mandarinko_scroller_item` WHERE `id`='".mysql_real_escape_string($_GET['deletep'])."'";
	$r = mysql_fetch_assoc(mysql_query($sql));

	$sql = "DELETE FROM `mandarinko_scroller_item` WHERE `id`='".mysql_real_escape_string($_GET['deletep'])."'";
	mysql_query($sql) OR die('DB ERROR: CAN\'T DEL PHOTO');
	@unlink($_SERVER['DOCUMENT_ROOT'].'/upload/scroller/'.$r['pid']."_".$_GET['deletep'].'.jpg');
	@unlink($_SERVER['DOCUMENT_ROOT'].'/upload/scroller/tmb/'.$r['pid']."_".$_GET['deletep'].'.jpg');
...
if($_POST['saveitem']) {
	...
	$sql = "UPDATE `mandarinko_scroller_item` SET
		`pid`	=	'".mysql_real_escape_string($_GET['item'])."',
		`title`	=	'".mysql_real_escape_string($_POST['title'])."',
		`text`	=	'".mysql_real_escape_string($_POST['text'])."',
		`coast`	=	'".mysql_real_escape_string($_POST['coast'])."',
	...
	$id = mysql_real_escape_string($_GET['editp']);
 	if($_FILES['img']['name']) {
		move_uploaded_file($_FILES['img']['tmp_name'], $_SERVER['DOCUMENT_ROOT'].'/upload/scroller/'.$_GET['item'].'_'.$id.'.jpg')
		or die("Ошибка закачки файла");
		copy($_SERVER['DOCUMENT_ROOT'].'/upload/scroller/'.$_GET['item'].'_'.$id.'.jpg',$_SERVER['DOCUMENT_ROOT'].'/upload/scroller/tmb/'.$_GET['item'].'_'.$id.'.jpg');

		SetImgSize($_SERVER['DOCUMENT_ROOT'].'/upload/scroller/tmb/'.$_GET['item'].'_'.$id.'.jpg',0,400, 1);
	}
	header('Location: /'.$URL[1].'/'.$URL[2].'/'.$URL[3].'?item='.$_GET['item']);
	exit;
}
...
if($_POST['additem']) {
	...
	$sql = "INSERT INTO `mandarinko_scroller_item` SET
		`pid`	=	'".mysql_real_escape_string($_GET['item'])."',
		`title`    = '".mysql_real_escape_string($_POST['title'])."',
		`text`    = '".mysql_real_escape_string($_POST['text'])."',
		`coast`    = '".mysql_real_escape_string($_POST['coast'])."',
...
<?if($_GET['editp']) {
	$sql = "SELECT * FROM `mandarinko_scroller_item` WHERE `id` = ".mysql_real_escape_string($_GET['editp']);
	$r = mysql_fetch_assoc(mysql_query($sql));?>
	...
	<table class="addtable">
		<tr>
			<td>Фотография</td>
			<td><img src="/upload/scroller/<?=$_GET['item'];?>_<?=$r['id'];?>.jpg?<?=time();?>" alt="" style="max-width:500px"/></td>
		</tr>
		<tr>
		  	<td>Сменить фото</td>
		...
		<tr>
			<td>Цена</td>
			<td><input type="text" class="long" name="coast" value="<?=$r['coast'];?>"/></td>
		</tr>
		<tr>
			<td>Ссылка</td>
...
<table class="addtable">
  ...
  <tr>
	<td>Цена</td>
	<td><input class="long" name="coast" value=""></td>
  </tr>
  <tr>
	<td>Ссылка</td>
...
<?$sql = "SELECT * FROM `mandarinko_scroller_item` WHERE `pid` = ".mysql_real_escape_string($_GET['item'])." ORDER BY `id`";
$r = mysql_query($sql);
...
foreach($data as $el) {?>
  <tr>
    <td><a class="tooltip" title="<?=$el['title'];?>"><img width="200" style="max-width:300px" src="/upload/scroller/tmb/<?=$el['pid'];?>_<?=$el['id'];?>.jpg?<?=time();?>"/></a></td>
    <td><?=$el['title'];?></td>
	<td><?=$el['text'];?></td>
	<td><?=$el['coast'];?></td>
    <td width="36">
